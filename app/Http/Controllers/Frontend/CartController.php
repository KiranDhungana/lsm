...
class CartController extends Controller{
        ...
        if (Auth::check()) {
            $items = $this->getAuthenticatedUserCartItems();
             foreach ($items as $key => $cart) {
                $carts[$key] = $this->formatCartItem($cart);
            }
        } else {
    ...
    {
        if (isModuleActive('Installment') && (isset($cart->is_installment) || isset($cart['is_installment']))) {
        ...
        if (isModuleActive('EarlyBird') && (isset($cart->is_early_bird) || isset($cart['is_early_bird']))) {
            return getPriceFormat(verifyEarlybirdOffer($course, Auth::id(), $cart)['price']);
        }
        if (isModuleActive('Store') && (isset($cart->is_store) || isset($cart['is_store']))) {
            return $this->getStoreItemPrice($cart);
        }
        return hasCouponApply($course->id)
            ? getPriceFormat(getCouponPrice($course->id))
            : getPriceFormat($course->discount_price ?: $course->price);
    }
